...
// Función para exportar a JSON y enviar por correo
function exportarJSONyEnviar() {
    // Verificar que el formulario esté completo
    if (!validarFormulario()) {
        alert('Por favor complete todos los campos requeridos antes de exportar los datos.');
        return;
    ...
    // Crear blob y descargar archivo JSON
    const blob = new Blob([jsonContent], { type: 'application/json' });
    const jsonFileName = `Cotizacion_${currentFolio}.json`;
...
Taller Félix`;
    ...
    // Mostrar mensaje de éxito
    document.getElementById('success-message').textContent = 'Archivo JSON generado. Por favor adjunte manualmente el archivo descargado al correo electrónico que se ha abierto.';
    const successModal = new bootstrap.Modal(document.getElementById('successModal'));
    successModal.show();
}
